...
`probability_ensemble` is the default validation flip solver mode for external vision-capable providers. Rehearsal lanes also force this path for remote providers so rehearsal does not silently exercise the older binary side-choice prompt.
