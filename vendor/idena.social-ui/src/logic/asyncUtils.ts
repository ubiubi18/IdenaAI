...
export const getPosterWithIndexerApi = async (indexerApiUrl: string, posterAddress: string) => {
    ...
    if (addressResult.error || identityResult.error || ageResult.error) {
        return;
    }

    const { address, stake } = addressResult.result;
    const { state } = identityResult.result;
    const age = ageResult.result;
...
export const processMessage = async (
    ...
    if (messageEvent.length !== 2 || !messageEvent[0] || !messageEvent[1]) {
        return { continued: true };
    }
