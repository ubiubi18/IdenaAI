...
function ProfileLikes() {
    ...
    const {
        ...
        profileActivity,
    } = useOutletContext() as ProfileLikesProps;

    return (<>
        <ul>
            {profileActivity.likes.map((likePostId: string) => {
                const likePost = postsRef.current[likePostId];
                const likedPost = postsRef.current[likePost.replyToPostId];
