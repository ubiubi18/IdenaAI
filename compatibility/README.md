...
Approval requires reviewed JSON reports for every `requiredGates` entry. Store
those reports under `compatibility/evidence/`. Each report must identify the same
...
Add one `gateResults` entry per required gate. Each entry must have status
`passed`, its repository-relative evidence path, and the report's SHA-256
digest. Then set `stack-lock.json` to `approved`.
`scripts/check-compatibility-lock.js` rejects missing, stale, duplicated,
path-escaping, symlinked, malformed, or checksum-mismatched evidence.

Do not approve from automated CI alone. The remaining live checks require:
