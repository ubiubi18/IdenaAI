...
const {loadAndVerifyGateResults} = require('./check-promotion-evidence')
...
const EVIDENCE_PATH_PATTERN =
  /^compatibility\/evidence\/[a-zA-Z0-9._/-]+\.json$/u
...
function readCanonicalJson(filePath) {
  const raw = readRegular(filePath)
  const value = JSON.parse(raw)
  if (`${JSON.stringify(value, null, 2)}\n` !== raw) {
    throw new Error(`${path.basename(filePath)} must use canonical JSON`)
  }
  return value
}
...
function verifyLockIdentity(lock) {
  if (
    lock.schema !== 1 ||
    lock.releaseId !== 'idena-mainnet-legacy-compat-2026.09.24-rc13' ||
...
function verifyCompatibilityLock(
  ...
  const {gateResults} = lock
  if (
    !gateResults ||
    typeof gateResults !== 'object' ||
    Array.isArray(gateResults) ||
    Object.keys(gateResults).some((gate) => !gates.includes(gate))
  ) {
    throw new Error('Compatibility lock contains invalid gate results')
  }
  for (const [gate, result] of Object.entries(gateResults)) {
    const {status, evidence, sha256} = result || {}
    if (
      !result ||
      typeof result !== 'object' ||
      Array.isArray(result) ||
      Object.keys(result).sort().join(',') !== 'evidence,sha256,status' ||
      status !== 'passed' ||
      !EVIDENCE_PATH_PATTERN.test(evidence || '') ||
      path.posix.normalize(evidence) !== evidence ||
      !SHA256_PATTERN.test(sha256 || '')
    ) {
      throw new Error(`Compatibility lock contains invalid results for ${gate}`)
    }
  }
  if (
    lock.status === 'approved' &&
    Object.keys(gateResults).length !== gates.length
  ) {
    throw new Error('Approved compatibility lock has incomplete gate results')
  }
...
function requireApproved(lock, verifyEvidence = loadAndVerifyGateResults) {
  if (lock.status !== 'approved') {
    throw new Error(
      'Compatibility stack is not approved; release promotion gates have not been completed'
    )
  }
  verifyEvidence(lock)
}
...
function main(argv = process.argv.slice(2)) {
  ...
  const lock = readCanonicalJson(LOCK_PATH)
  ...
  if (lock.status === 'approved' || options.requireApproved) {
    requireApproved(lock)
  }
