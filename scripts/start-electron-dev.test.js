...
describe('Electron renderer readiness', () => {
  ...
  it('warms every configured renderer route before Electron starts', async () => {
    ...
    expect(isReady.mock.calls[2][0].url).toBe(
    ...
    expect(isReady.mock.calls[3][0].url).toBe(
    ...
    expect(isReady.mock.calls[4][0].url).toBe('http://127.0.0.1:8000/ai-chat')

    readinessResolvers.shift()(true)
    await Promise.resolve()
    expect(isReady).toHaveBeenCalledTimes(6)
    expect(isReady.mock.calls[5][0].url).toBe('http://127.0.0.1:8000/social')
